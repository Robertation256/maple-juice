...
var candidateId string = membership.SelfNodeId
...
func StartLeaderElectionServer() {
	util.MEMBERSHIP_SERVER_STARTED.Wait()
	...
	util.LEADER_ELECTION_SERVER_STARTED.Done()
...
func handleElectionMsg(conn *net.UDPConn, msg *ElectionMessage) {
	...
	if msg.MessageType == ELECTION_REQUEST {
		...
		if len(voterSet) >= quorumSize {
			LeaderId = membership.SelfNodeId
...
func multicast(conn *net.UDPConn, payload []byte) {
	aliveMembers := membership.LocalMembershipList.AliveMembers()
